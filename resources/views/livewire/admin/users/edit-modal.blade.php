        ...
        <div  x-transition:enter="ease-out duration-300"
            ...
            <div class="w-full">
                <div class="mt-3 text-center">
                    ...
                    <div class="mt-2">
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 grid grid-cols-2 gap-4">
                            <div class="">
                                <div>
                                    <div class="mb-2">
                                        <label for="username" class="block text-gray-700 text-sm font-bold mb-2 text-left">Username:</label>
                                        <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                            id="username" wire:model.defer="user.username" type="text" wire:disabled="action == edit">
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                            id="firstname" wire:model.defer="user.firstname" type="text">
                                            @error('user.firstname') <span class="text-red-500">{{ $message }}</span>@enderror
                                    </div>

                                    <div class="mb-2">
                                        <label for="lastname" class="block text-gray-700 text-sm font-bold mb-2 text-left">Last Name:</label>
                                        <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                            id="lastname" wire:model.defer="user.lastname" type="text">
                                            @error('user.lastname') <span class="text-red-500">{{ $message }}</span>@enderror
                                    </div>

                                    <div class="mb-2">
                                        <label for="email" class="block text-gray-700 text-sm font-bold mb-2 text-left">Email:</label>
                                        <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                            id="email" wire:model.defer="user.email" type="email">
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                            id="status" wire:model.defer="user.status" type="status">
                                            <option value="">Choose Status<option>
                                            @foreach(config('project.user_status') as $key => $value)
                                            <option value="{{ $key }}">{{ $value }}</option>
                                            @endforeach
                                        </select>
                                            @error('user.status') <span class="text-red-500">{{ $message }}</span>@enderror
                                    </div>
                                </div>
                            </div>
                            <div>
                                <div class="accordion" id="accordionExample">
                                    @if ($action == 'create')
                                    <div x-data={show:true}  class="accordion-item bg-white border border-gray-200">
                                    @else
                                    <div x-data={show:false}  class="accordion-item bg-white border border-gray-200">
                                    @endif
                                        <h2 class="accordion-header mb-0 flex" id="heading__item1">
                                            <button @click="show=!show" class=" accordion-button relative flex items-center w-1/2 py-4 px-5 text-base text-gray-800 text-left bg-white border-0 rounded-none transition focus:outline-none"
                                                type="button" data-bs-toggle="collapse" data-bs-target="#collapse__item1" aria-expanded="true"
                                                aria-controls="collapse__item1">
                                                Password
                                            </button>

                                        </h2>
                                        <div x-show="show" id="collapse__item1" class="accordion-collapse collapse show" aria-labelledby="heading__item1"
                                            ...
                                            <div class="accordion-body py-4 px-5">
                                                <div class="w-full">
                                                    <div class="mb-2">
                                                        ...
                                                        <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                                            id="password" wire:model.defer="password" type="password">
